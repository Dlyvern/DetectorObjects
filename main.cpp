...
private:
    ...
    int m_DelayForKey{10};

    cv::dnn::Net m_CarDetectionModel;
    ...
    static cv::dnn::Net GetModelForCarDetection()
    {
        std::string weights_path{"../YOLO/yolov3-tiny_final.weights"};
        std::string config_path{"../YOLO/yolov3-tiny.cfg"};
    ...
    void FindCars(cv::Mat& image)
    {
        ...
        m_CarDetectionModel.setInput(blob);

        std::vector<cv::Mat>outs;

        m_CarDetectionModel.forward(outs, m_CarDetectionModel.getUnconnectedOutLayersNames());
...
public:
    ...
    void Start()
    {
        ...
        m_CarDetectionModel = GetModelForCarDetection();
        m_IsWorking = true;
